...
def OMAC(k1, k2, k3, m):
    ##TODO: message will need to be converted to byte to work## <- or does it? am i passing in a bytes an assuming it'll all work? confirm
    block, rest = peel(m)
    #check if last block
    if rest != []:
        #create first Enc block
        enc1 = blockEncrypt(k1, block)
        ...
        lastblock(k1, k2, k3, block, [])


def lastblock(k1, k2, k3, m, enc1):
